...
var RestapiService = /** @class */ (function () {
    function RestapiService(cognitoService, http) {
    ...
    RestapiService.prototype.getData = function () {
        var _this = this;
        var myUser = this.cognitoService.getAuthenticatedUser();
        if (myUser === null) {
            console.log("user is null");
            return;
        }
        myUser.getSession(function (err, session) {
            if (err) {
                console.log("get error: ", err);
                return;
            }
            console.log('get session: ', session);
            var token = session['idToken']['jwtToken']; // session.getIdToken().getJwtToken();
            ...
            _this.http.post('https://ceo7e027k2.execute-api.us-east-2.amazonaws.com/newtestStage/Realgetdata', JSON.stringify(postmoreData))
                .subscribe(function (response) {
                console.log(response);
                _this._data = response;
                console.log("get success: ", _this._data);
                // setTimeout(()=>{
                _this.User.spadescore = _this._data.Item.User.M.spadescore.N;
                _this.User.starswin = _this._data.Item.User.M.starswin.N;
                _this.User.holopoint = _this._data.Item.User.M.holopoint.N;
                _this.User.starsvideo = _this._data.Item.User.M.starsvideo.BOOL;
    ...
    RestapiService.prototype.postData = function () {
        var _this = this;
        var myUser = this.cognitoService.getAuthenticatedUser();
        if (myUser === null) {
            console.log("user is null");
        ...
        myUser.getSession(function (err, session) {
            ...
            // https://499adbe4a1.execute-api.us-east-2.amazonaws.com/dev/postToTable
            _this.http.post('https://ceo7e027k2.execute-api.us-east-2.amazonaws.com/newtestStage/petfood', JSON.stringify(postData), { headers: myHeaders })
                .subscribe(function (response) {
    ...
    RestapiService = __decorate([
        ...
        __metadata("design:paramtypes", [_cognito_service__WEBPACK_IMPORTED_MODULE_1__["CognitoService"], _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpClient"]])
    ], RestapiService);
